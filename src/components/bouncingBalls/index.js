class Ball {
  constructor(svg, id, color, mass, x0, y0, vx0, vy0, dt = 50, scale = 1) {
    this.x = x0;
    this.y = y0;
    this.vx = vx0;
    this.vy = vy0;
    ...
    this.dt = dt;
    this.color = color;
    this.radius = mass; // add radius and density to
    this.svg = svg;
    this.data = [this.id];
    /* arrow functions bind to the context, try using them:
      draw = () => {}
    */
  ...
  move() {
    this.x += this.vx;
    this.y += this.vy;
    // horizontal axis boundary conditions
    if (parseFloat(this.svg.attr("width")) <= this.x + this.radius) {
      this.x = parseFloat(this.svg.attr("width")) - this.radius - 1;
      this.vx = -this.vx;
    }
    ...
    // vertical axis boundary conditions
    if (parseFloat(this.svg.attr("height")) < this.y + this.radius) {
      this.y = parseFloat(this.svg.attr("height")) - this.radius - 1;
      this.vy = -this.vy;
    }
